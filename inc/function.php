...
function combo_user($kode,$MessID) {
	echo "<option value='' selected>- Select User -</option>";
	$query = query("SELECT UserID,UserName FROM UserInfo where MessID='$MessID' ");
	while ($row = mysql_fetch_row($query)) {
		if ($kode == "")
			echo "<option value='$row[0]'> " . ucwords($row[1]) . " </option>";
		else
...
function combo_user_type($kode) {
	echo "<option value='0' selected>-  User Type -</option>";
	$hari=array('manager','member');
	foreach($hari as $value){
		if ($kode == "")
			echo "<option value='$value'> " . ucwords($value) . " </option>";
		else
...
function combo_poi($kode) {
	...
	$tahun = array('Police', 'FillingStation', 'Hospital');
	foreach($tahun as $key => $value) {
		if($kode == "")
			echo "<option value='$value'> " . ucwords($value) . " </option>";
		else
